    ...
    void Update()
    {
        ...
        int row = 1;
            for (int i = 0; i < displayBins; i++)
            {
                ...
                Vector3 dir = (basePos - origin).normalized;
                bars[row, i].transform.position = basePos + dir * (amp * 0.5f);
                bars[row, i].transform.up = dir;

                bars[row, i].transform.Rotate(Vector3.up * 20f * Time.deltaTime);
   ...
   void ChangeColor()
    {
        ...
        // Sum linear amplitudes
        for (int i = 0; i < displayBins; i++)
        {
            int linearBin = mappedBins[LinearRow, i];
            totalIntensity += AudioSpectrum.samples[linearBin];
        }

        // Normalize intensity (tweak multiplier as needed)
        float normalized = Mathf.Clamp01(totalIntensity * 5f);

        // Map intensity to hue
        Color color = Color.HSVToRGB(0.33f,normalized * 2f,normalized * 2f);

        // Apply to entire ring
        for (int i = 0; i < displayBins; i++)
        {
            bars[1, i].GetComponent<Renderer>().material.color = color;
        }
    }
    ...
    {
        float radius = plotWidth * 0.3f;

        float t = displayIndex / (float)(displayBins - 1f);
